...
- Discovery: `period_lists?filter[type]={monthly|average|spot}&year=YYYY` (JSON:API), used only by the update script.
  The crate computes file URLs and treats 404 as "not published yet"..
...
- Codes are as published by HMRC, which is not always ISO 4217.
  E.g. Ecuador appears as `ECS` long after dollarization.

## Build-time validation

The build script re-parses everything on every build and fails on:

- non-calendar-month periods,
- gaps in the monthly series,
- malformed codes,
- non-positive rates,
- out-of-range mantissas,
- non-March/December spot or average periods,
- conflicting duplicates with no majority.
